                ...
                <div className='row'>
                  {Myorders.map((order, orderIndex) => (
                    <div className='my-4' style={{ background: "#f5f5f5", boxShadow:"0px 4px 8px rgba(0, 0, 0, 0.3)", borderRadius:"10px" }}>
                      <div className='col-12 d-lg-flex  pt-2 ps-lg-3' key={orderIndex}>
                        <div className='col-lg-5 col-12'>
                          ...
                          <div className='mt-lg-3'>
                            <div className=''>
                              <p><i style={{ fontWeight: "600" }}>Name:</i> {order.formData.fullName}</p>
                              <p><i style={{ fontWeight: "600" }}>Email:</i> {order.formData.emailAddress}</p>
                              <p><i style={{ fontWeight: "600" }}>Phone:</i> {order.formData.phoneNumber}</p>
                            </div>
                            <div><i style={{ fontWeight: "600" }}>Street Address:</i> {order.formData.streetAddress}</div>
                            <div className='d-flex gap-4 mt-3'>
                              <p><i style={{ fontWeight: "600" }}>City:</i> {order.formData.city}</p>
                              <p><i style={{ fontWeight: "600" }}>State:</i> {order.formData.state}</p>
                              <p><i style={{ fontWeight: "600" }}>Pin Code:</i> {order.formData.pinCode}</p>
                            </div>
                          </div>
                        </div>
                      ...
                      <h4 className='mb-3 ps-3'><i>Total Payable Amount:</i> ₹{order.totalPrice}</h4>
                    </div>
                  ))}
                </div>
              )}
